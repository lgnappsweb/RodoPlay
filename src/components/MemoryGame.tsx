...
export function MemoryGame({ onComplete, onScoreUpdate, onCancel, currentPlayerId }: MemoryGameProps) {
  ...
  useEffect(() => {
    if (!setupComplete || isTimeOut) return;
    if (board.length > 0 && board.every(c => c.matched)) return;
  ...
  }, [timeLeft, setupComplete, isTimeOut, board]);
    ...
    if (newSelection.length === 2) {
      ...
      if (board[first].icon === board[second].icon) {
        setTimeout(() => {
          ...
          // Time bonus for finishing fast
          const isFinished = matchedBoard.every(c => c.matched);
          const timeBonus = isFinished ? Math.round(timeLeft * 10 * multiplier) : 0;
          const totalEarned = points + timeBonus;

          if (multiplayerMode === '2p') {
            if (activePlayerTurn === 'p1') {
              setP1Score(s => s + totalEarned);
            } else {
              setP2Score(s => s + totalEarned);
            }
          } else {
          ...
          if (isFinished) {
            setTimeout(() => {
              const finalP1 = activePlayerTurn === 'p1' ? p1Score + totalEarned : p1Score;
              const finalP2 = activePlayerTurn === 'p2' ? p2Score + totalEarned : p2Score;
              onComplete(
                multiplayerMode === '2p' ? finalP1 : score + totalEarned,
                1,
                multiplayerMode === '2p',
                selectedPartner,
                finalP1,
    ...
    <div className="min-h-screen bg-slate-950 p-6 flex flex-col items-center space-y-6">
      ...
      <div className="w-full flex justify-between items-center mb-6">
        ...
        <div className="flex flex-col items-center">
          <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Tempo</p>
          <p className={`text-xl font-black font-mono transition-colors ${timeLeft <= 10 ? 'text-red-500 animate-pulse bg-red-950/20 px-2.5 py-0.5 rounded-lg border border-red-500/30' : 'text-white'}`}>
            {timeLeft}s
          </p>
        </div>
      ...
      {/* Visual Timer Bar */}
      <div className="w-full max-w-lg bg-slate-900 h-2 rounded-full overflow-hidden border border-slate-800 relative mb-4">
        <motion.div 
          className={`h-full ${timeLeft <= 10 ? 'bg-red-500 shadow-[0_0_15px_rgba(239,68,68,0.5)]' : 'bg-yellow-400 shadow-[0_0_15px_rgba(251,191,36,0.3)]'}`}
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <p className="text-slate-400 text-xs leading-relaxed italic">
                  O cronômetro esgotou antes de você encontrar todos os pares de placas. Resgate o XP acumulado ou tente reconfigurar:
                </p>
              </div>

              {/* Score box */}
              <div className="w-full bg-slate-950/60 p-4 rounded-2xl border border-slate-850">
                <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest block mb-1 font-sans">Pontuação Conquistada</span>
                <span className="text-3xl font-black text-yellow-400 font-mono block">{score} XP</span>
              </div>

              <div className="w-full flex flex-col gap-3">
                <Button 
                  onClick={() => onComplete(score, 1, false, null, 0, 0, 'MEMORY', true)} 
                ...
                <Button 
                  ...
                  className="w-full h-12 bg-slate-950 hover:bg-slate-900 text-white font-bold text-xs rounded-2xl border border-slate-800 uppercase tracking-wider active:scale-95 transition-all"
                >
                  TENTAR NOVAMENTE 🔁
                </Button>
                <Button 
